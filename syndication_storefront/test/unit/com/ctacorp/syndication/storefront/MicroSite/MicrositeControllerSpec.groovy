...
//    ------Current List --------
    ...
    def "currentList should find collections"() {
        ...
        then: "response should contain the name of the collection"
            response.text.contains("myCollection")
    }
...
//    ---------------- Specific List -------------
    ...
    def "searchTags should render tags as JSON"() {
        setup:
            controller.tagService.metaClass.listTags = {Map map -> return [tags:[name:"test"]]}
    ...
    def "displayStyle should render the fullPagePagination template"() {
        setup:"param values"
            controller.micrositeService.metaClass.getMediaContents = {def items -> return [1]}
            params.offset = "0"
            params.mediaItems = "1"
        when:"summary is called"
