...
class GridControllerSpec extends Specification {
    ...
    def setup() {
        ...
        micrositeService.saveBuild(_, _) >> { params, micrositeType -> return new MicroSite() }
    }

    def populateValidParams(params) {
        assert params != null
        //mediaItem required attributes
        params["templateType"] = MicroSite.TemplateType.GRID
    ...
    void "Save should save the microsite and redirect to the show page"() {
        ...
        then: "the microsite should get saved and be redirected to its show page"
            response.redirectedUrl == "/grid/show?showAdminControls=true"
    ...
    void "calling show should render the show view"() {
        ...
        then: "get the media items"
            1 * micrositeService.getMediaItems(micro.mediaArea3) >> {a -> return ["item"] }
            1 * micrositeService.getMediaItems(micro.mediaArea2) >> {a -> return ["item"] }
            1 * micrositeService.getMediaItems(micro.mediaArea1, 15) >> {a, b -> return ["item"] }
