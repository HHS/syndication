...
class MediaServiceSpec extends Specification {
    ...
    def "mediaItem elastic Search should return a list of media items"() {
        given: "a mocked elastic service"
            service.elasticsearchService = [search:{t ->[MediaItem.first().id] }]
        when: "elastic search is called"
            def results = service.elasticsearchService.search("someQueryString")
        then:
            results == [MediaItem.first().id]
    }
