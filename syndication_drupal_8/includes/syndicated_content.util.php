...
$moduel_path = $module_handler->getModule('syndicated_content')->getPath();

require_once($moduel_path . '/lib/Syndication.lib.php');
...
function _syndicated_content_api_sources()
{
    ...
    $results = $conn->select('syndicated_content_sources', 's')
                 ->fields('s')
...
function _syndicated_content_type_mapping( $source_id=null )
{
    ...
    $first_source = $conn->select('syndicated_content_sources','s')
                        ->fields('s')
                        ->range(0,1)
                        ->execute();
    ...
    $results = $conn->select('syndicated_content_types', 't')
                 ->fields('t')
...
function _syndicated_content_get_source_id()
{
    ...
    $sources = $conn->select('syndicated_content_sources','s')
        ->fields('s')
        ->range(0,1)
        ->execute();
...
function _syndicated_content_build_metadata_view( $metadata )
{
    ...
    switch ( strtolower($metadata['mediaType']) )
    {
        ...
        default:
            $html = "
              <script type=\"text/javascript\">jQuery('#syndication_media_id').val(". \Drupal\Component\Serialization\Json::encode(\Drupal\Component\Utility\SafeMarkup::checkPlain($metadata['id'])) .");</script>
              <table id=\"lookup_metadata\">
              <tbody>
                <tr>
                  <th>Name</th>          <td>". \Drupal\Component\Utility\SafeMarkup::checkPlain($metadata['name']) ."</td>
                  ...
                  <th>Source</th>  <td>". \Drupal\Component\Utility\SafeMarkup::checkPlain($metadata['source']['name']) ."</td>
                </tr><tr>
                  ...
                  <th>Id</th>            <td colspan=\"2\">". \Drupal\Component\Utility\SafeMarkup::checkPlain($metadata['id'])      ."</td>
                </tr><tr>
