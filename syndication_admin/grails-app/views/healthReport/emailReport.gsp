...
<body>
  <div>
    ...
    <div class="banner"><div class="logo"><img src="${grails.util.Holders.config.grails.serverURL}/assets/smallSyndicationLogo.png"/></div><span>HHS Media Services</span></div>
    <div class="clearfix"></div>
    <div class="message_body">
      <h1>The following media items were identified as problematic in the Syndication System</h1>
      <p>Please verify these items, and correct as needed. You can validate the items in the <g:link controller="healthReport" action="index" absolute="true">Syndication Admin Dashboard</g:link></p>
      <p>Flagged Media Items:</p>
      <ul>
        <g:each in="${flaggedMediaItems}" var="flaggedMediaInstance">
          <li><strong><g:link controller="mediaItem" action="show" id="${flaggedMediaInstance.mediaItem.id}" absolute="true">${flaggedMediaInstance.mediaItem.name}</g:link></strong> (${flaggedMediaInstance.mediaItem.id})
            <ul>
              <li><Strong>Error:</Strong> ${flaggedMediaInstance.failureType.prettyName}</li>
              <li><Strong>Description:</Strong> <g:message code="healthReport.failure.${flaggedMediaInstance.failureType}"/></li>
            </ul>
          </li>
        </g:each>
      </ul>
