...
class MediaValidationServiceSpec extends Specification {
    ...
    def messageSource = new MockFor(PluginAwareResourceBundleMessageSource)
    def mediaItemsService = new MockFor(MediaItemsService)
    def springSecurityService = new MockFor(SpringSecurityService)
    def publishmediaitemcnt = 0
    //def grailsApplication = new MockFor(GrailsApplication)
...
//            response.details == "process called"
    ...
    def "test full health scan"() {
        ...
        when:"full health scan is called with a subscriberId"
            service.mediaItemsService = [getPublisherMediaIds: { Long subscriberId -> publishmediaitemcnt++ }]
            service.fullHealthScan(1)

        then:""
            publishmediaitemcnt == 1
