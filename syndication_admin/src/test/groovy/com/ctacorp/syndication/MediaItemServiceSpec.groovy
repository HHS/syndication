...
@Mock([AlternateImage, User, UserRole, Role, MediaItem, SystemEvent, Html, Video, Image, Infographic, QuestionAndAnswer,Collection, Tweet, PDF, MediaItemSubscriber, Campaign, MediaPreview, MediaThumbnail, CachedContent, FlaggedMedia, ExtendedAttribute, MediaMetric, UserMediaList])
...
//                LinkedHashMap.metaClass.count = {}
    ...
    def "saving a mediaItem works properly when saving/updating as a publisher"(){
        ...
        then: "index the media item"
        1 * service.elasticsearchService.indexMediaItem(mi)
...
// -------------- DELETE ---------------
    def "A mediaItem gets deleted along with all of its dependencies"() {
        ...
        then:"delete the content item from tag cloud and elasticsearch"
        1 * service.tagService.removeContentItem(mi.id)
        1 * service.elasticsearchService.deleteMediaItemIndex(mi.id, mi.class.name)
...
//    Add extended Attribute
    ...
    def "test adding a valid extended attribute"() {
        ...
        service.extendedAttributeService = [getUpdateInformation: {ExtendedAttribute att ->
            def name = att.name
